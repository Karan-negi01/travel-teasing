    ...
    <section className="py-8 sm:py-10">
      ...
      <div className="max-w-[90vw] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-4 sm:mb-5">
          ...
          <Link
            href="/treks"
            className="hidden sm:inline-flex shrink-0 items-center gap-2 rounded-xl border border-emerald-500/80 bg-white px-4 py-2.5 text-sm font-semibold text-emerald-700 shadow-sm hover:bg-emerald-500 hover:text-white transition-colors"
          >
            View all
        ...
        <div className="max-w-[90vw] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-stretch gap-4 sm:gap-5 pb-2 pr-4 sm:pr-6 lg:pr-8 snap-x snap-mandatory">
            {featuredTreks.map((trek, index) => (
                ...
                key={trek.id}
                href={`/treks/${trek.id}`}
                  className="snap-start w-[62vw] sm:w-[46vw] md:w-[36vw] lg:w-[260px] xl:w-[280px] flex-shrink-0 group rounded-2xl overflow-hidden border border-gray-200/80 bg-white shadow-sm hover:shadow-lg hover:border-emerald-200/60 transition-all duration-300"
              >
      ...
      <div className="sm:hidden max-w-[90vw] mx-auto px-4 sm:px-6 lg:px-8 mt-4 flex justify-center">
        <Link
          href="/treks"
          className="inline-flex items-center justify-center gap-1.5 rounded-[3px] border border-emerald-500/80 bg-white px-4 py-2 text-xs font-semibold text-emerald-700 shadow-sm hover:bg-emerald-500 hover:text-white transition-colors"
        >
          View all
          <span aria-hidden>→</span>
        </Link>
